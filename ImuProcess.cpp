...
void ImuProcess::UndistortPcl(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI &pcl_out)
{
  ...
  const double &imu_beg_time = v_imu.front()->header.stamp;
  const double &imu_end_time = v_imu.back()->header.stamp;
  ...
  for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++)
  {
    ...
    if (tail->header.stamp < last_lidar_end_time_)    continue;
    ...
    if(head->header.stamp < last_lidar_end_time_)
    {
      dt = tail->header.stamp - last_lidar_end_time_;
      // dt = tail->header.stamp.toSec() - pcl_beg_time;
    }
    else
    {
      dt = tail->header.stamp - head->header.stamp;
    }
    ...
    double &&offs_t = tail->header.stamp - pcl_beg_time;
...
void ImuProcess::Process(const MeasureGroup &meas,  esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI::Ptr cur_pcl_un_)
{
    ...
    std::cout << "undistortPcl" << std::endl;
