...
class EzQVTKInteractorInternal;

/**
 * @class EzQVTKInteractor
 * @brief - an interactor for QVTKOpenGLNativeWidget (and QVTKWiget).
 *
 * EzQVTKInteractor handles relaying Qt events to VTK.
 * @sa QVTKOpenGLNativeWidget
 */

class EzQVTKInteractor : public vtkRenderWindowInteractor {
public:
  static EzQVTKInteractor *New();
  vtkTypeMacro(EzQVTKInteractor, vtkRenderWindowInteractor);
...
private:
  EzQVTKInteractorInternal *Internal;

  EzQVTKInteractor(const EzQVTKInteractor &) = delete;
  void operator=(const EzQVTKInteractor &) = delete;
};
