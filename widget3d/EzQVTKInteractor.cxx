...
#include "EzQVTKInteractorInternal.h"
...
EzQVTKInteractorInternal::EzQVTKInteractorInternal(EzQVTKInteractor *p)
    : Parent(p) {
  this->SignalMapper = new QSignalMapper(this);
#if (QT_VERSION >= QT_VERSION_CHECK(5, 15, 0))
  QObject::connect(this->SignalMapper, &QSignalMapper::mappedInt, this,
...
EzQVTKInteractorInternal::~EzQVTKInteractorInternal() = default;

void EzQVTKInteractorInternal::TimerEvent(int id) { Parent->TimerEvent(id); }

/*! allocation method for Qt/VTK interactor
 */
vtkStandardNewMacro(EzQVTKInteractor);

/*! constructor for Qt/VTK interactor
 */
EzQVTKInteractor::EzQVTKInteractor() {
  this->Internal = new EzQVTKInteractorInternal(this);
...
void EzQVTKInteractor::Initialize() {
#if defined(VTK_USE_TDX) && defined(Q_OS_WIN)
  if (this->UseTDx) {
...
//------------------------------------------------------------------------------
void EzQVTKInteractor::StopListening() {
#if defined(VTK_USE_TDX) && defined(Q_OS_WIN)
  if (this->Device->GetInitialized() && this->Device->GetIsListening()) {
...
/*! handle timer event
 */
void EzQVTKInteractor::TimerEvent(int timerId) {
  if (!this->GetEnabled()) {
    return;
  }
 ...
 */
int EzQVTKInteractor::InternalCreateTimer(int timerId,
  ...
  this->Internal->Timers.insert(
      EzQVTKInteractorInternal::TimerMap::value_type(platformTimerId, timer));
  return platformTimerId;
}

/*! destroy timer
 */
int EzQVTKInteractor::InternalDestroyTimer(int platformTimerId) {
  EzQVTKInteractorInternal::TimerMap::iterator iter =
